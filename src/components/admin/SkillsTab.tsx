...
export default function SkillsTab({ data, setData }: SkillsTabProps) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-3">
          <div className="relative group">
            ...
            <input
              ...
              className="bg-black/50 border border-white/10 rounded px-3 py-1.5 pl-9 text-sm text-white focus:outline-none focus:border-neon-cyan/50 w-48 lg:w-64"
            />
          </div>
          <button
      ...
      {/* Skills Grid */}
      <div className="bg-[#0B1220]/50 backdrop-blur border border-white/10 rounded-xl p-6 min-h-[500px] flex flex-col justify-between">
        {paginatedSkills.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center text-white/40 font-mono text-sm gap-6">
            <span>NO_MODULES_FOUND</span>
            <button
              onClick={loadDefaultSkills}
              className="px-6 py-2 bg-neon-cyan/10 border border-neon-cyan/30 text-neon-cyan rounded-md hover:bg-neon-cyan/20 transition-all font-orbitron text-sm tracking-wider shadow-[0_0_15px_rgba(0,245,255,0.1)]"
            >
            ...
            {paginatedSkills.map((skill, i) => {
              ...
              return (
                <motion.div
                  ...
                  key={`${skill.name}-${i}`}
                  className={`bg-black/40 border border-white/5 rounded-lg p-4 group transition-all relative overflow-hidden ${
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <h3 className="font-orbitron font-bold text-white tracking-wider truncate pr-4">
                      {skill.name}
                    </h3>
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity absolute right-2 top-2 bg-black/80 rounded p-1">
                      <button
                        onClick={() => toggleVisibility(origIndex)}
                        className={`p-1 transition-colors ${skill.visible !== false ? "text-neon-cyan/50 hover:text-neon-cyan" : "text-white/30 hover:text-white"}`}
                      >
                  ...
                  {/* Progress Bar */}
                  <div className="w-full bg-black/60 rounded-full h-1.5 mt-2 border border-white/5 overflow-hidden">
                    <div
      ...
      >
        {editingSkill && (
          <div className="space-y-4">
            ...
            <label className="flex items-center gap-3 cursor-pointer p-3 bg-black/30 border border-white/5 rounded hover:bg-black/50 transition-colors w-fit">
              <input
                type="checkbox"
            ...
            <div>
              <label className="block text-xs font-mono text-white/50 mb-1 flex justify-between">
            ...
            <div className="flex justify-end gap-3 pt-4 mt-6 border-t border-white/10">
              ...
              <button
                ...
                className="px-6 py-2 rounded text-sm font-bold bg-neon-cyan/20 text-neon-cyan border border-neon-cyan/50 hover:bg-neon-cyan/30 transition-all shadow-[0_0_10px_rgba(0,245,255,0.1)] disabled:opacity-50"
              >
                Save Module
              </button>
            </div>
